...
int main()
{
    ...
    // remember to count the errors, if any
     ...
     {
         bool exista = filesystem::exists(args[0]);
         if (exista)
         {
             fstream fisier;
             fisier.open(args[0], ios::app);
    ...
    // do not capture any reference for this lambda
     auto times = [](vector<string> comanda)
     {
         int nr_ap = 0;
    ...
    // remember to count the errors, if any
     ...
     {
         bool exista1 = filesystem::exists(args[0]);
         bool exista2 = filesystem::exists(args[1]);
         if (!exista1 && !exista2)
         {
             number_of_errors += 2;
             printf("Niciunul dintre fisiere nu exista!\n");
         }
         else
             if (!exista1 || !exista2)
             {
             ...
             else
             {
                 ...
                 fisier_intrare.open(args[0], ios::in);
                 fisier_iesire.open(args[1], ios::out);

                 string continut = "";
                 int i;
                 for (i = 0; !fisier_intrare.eof(); i++)
                 {
                     continut += fisier_intrare.get();
                 }
                 fisier_intrare.close();
                 
                 continut.erase(continut.end() - 1);
                 i--;
                 fisier_iesire << continut;
                 fisier_iesire.close();
             }
     };
